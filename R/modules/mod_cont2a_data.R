# ======================================================
# Continuous 2-arm: Data & Priors (Updated UX)
# ======================================================
...
cont2a_data_ui <- function(id) {
  ...
  tabItem(
    tabName = "cont2a",
    fluidRow(
      # Configuration Box - Matching Binary UX
                 ...
                 wellPanel(
                   ...
                   fluidRow(
                     column(6,
                            ...
                            numericInput(ns("n_c"), "Total N (n_c)", 30, min = 2)
                     ),
                     column(6,
                            h5(tags$b("Treatment Arm")),
                            numericInput(ns("mean_t"), "Mean", 0.5),
                            numericInput(ns("sd_t"), "SD", 1, min = 0.001),
                            numericInput(ns("n_t"), "Total N (n_t)", 30, min = 2)
                     )
                   )
                 )),
          column(7,
    ...
    # Executive Summary Boxes - Matching Binary UX
    fluidRow(
    ...
    # Diagnostics - Matching Binary Side-by-Side Layout
    fluidRow(
...
cont2a_data_server <- function(id, app_rv) {
  moduleServer(id, function(input, output, session) {
    ...
    # Defensive Reset: Clears plots/ESS when inputs change until re-built
    ...
    observeEvent(input$build_all, {
      ...
      validate(
        ...
        need(input$sd_c > 0 && input$sd_t > 0, "Standard deviations must be positive.")
      )

      # 1. Generate Priors (Control = Borrowed, Treatment = Flat)
      ...
      app_rv$cont_current <- list(
        mean_c = input$mean_c, sd_c = input$sd_c, n_c = input$n_c,
        mean_t = input$mean_t, sd_t = input$sd_t, n_t = input$n_t
      )

      showNotification("Continuous analysis built successfully.", type = "message")
    })

    # --- Value Box Outputs ---
    output$ess_prior_box <- renderValueBox({
      req(local_rv$ess)
      valueBox(round(local_rv$ess$ess_prior, 1), "Control Prior ESS", icon = icon("history"), color = "blue")
    })

    output$ess_current_box <- renderValueBox({
      valueBox(input$n_c + input$n_t, "Total Trial N (C+T)", icon = icon("vial"), color = "purple")
    })
    ...
    output$mu_txt <- renderText({
      ...
      paste0("Mean Posterior | Mean: ", round(local_rv$active_post$m, 4),
